...
import com.atul.spark.MyKryoRegistrator
import org.apache.hadoop.conf.Configuration
import org.apache.hadoop.fs.FileSystem
import org.apache.spark.sql.SQLContext
import org.apache.spark.{SparkConf, SparkContext}

//Define Object for weblogs
object WebLogChallenge {
  ...
  //converting the datetime to epoch date to  calculate session durations
  ...
  //parsing each log records by defining parseLogLine function
  def parseLogLine(log: String): WebRecord = {
    try {
      val res = PATTERN.findFirstMatchIn(log)

      if (res.isEmpty) {
        println("Reject logs records which are empty : " + log)
        WebRecord("Empty", "-", "-", "", "",  "", "", "-", "-","","","","","","" )
      }
      ...
      else
      {
        ...
        {
          WebRecord(m.group(1), m.group(2), m.group(3),m.group(4), m.group(5), m.group(6), m.group(7), m.group(8), m.group(9), m.group(10), m.group(11), m.group(12), m.group(13), m.group(14), m.group(15))
        }
      }
    ...
    {
      case e: Exception =>
        println("Please check log records it seems some problem:" + log + ":" + e.getMessage);
        WebRecord("Empty", "-", "-", "", "",  "", "", "-", "-","","","","","","" )
    }
  }

  //Main method to start execution of weblogs
  def main(args: Array[String]) =
  {
    // DONE - master (local or YARN) is set while submitting the job
    val sparkConf = new SparkConf().setAppName("Start Spark process")
    ...
    // Same as HDFS block size
    val blockSize = 1024 * 1024 * 256

    /* Intialiazed spark context and block size values */
    ...
    //import sql package and methods
    import sqlContext.implicits._
    import org.apache.spark.sql.expressions.Window
    import org.apache.spark.sql.functions._

    //val loglines = sc.textFile("hdfs://quickstart.cloudera:8020/user/cloudera/paytm/data/paypay.log")

    //converting web log file into RDD
    ...
    //filtered out data which is not as session logs and pick only timestamp, userIp, url,user_agent, epoch
    ...
    //User define function to concatenate two passed in string values
    ...
    //Removing nulls from epoch columns
    ...
    val webLogDfWithSessionId=webLogDfWithWindowIdx.withColumn("sessionId",getConcatenated($"userIp",$"windowIdx")).select($"userIp",$"sessionId",$"duration_miliseconds",$"url",$"userAgent").cache()
